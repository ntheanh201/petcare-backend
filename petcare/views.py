...
class PostViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        for post in filtered_posts:
            ...
            info = {
                ...
                "assigned_user_avatar": request.build_absolute_uri(
                    post.assigned_user.avatar.url
                ),
                ...
                "photos": list(
                    map(
                        lambda x: request.build_absolute_uri(x.img_url.url),
                        post.photos.all(),
                    )
                ),
                "reaction_id": Reaction.objects.get(
                    assigned_post=post, assigned_user=user
                ).id
                if len(Reaction.objects.filter(assigned_post=post, assigned_user=user))
                > 0
                else -1,
            }
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        post_info = {
            ...
            "photos": list(
                map(
                    lambda x: "http://" + request.get_host() + x.img_url.url,
                    post.photos.all(),
                )
            ),
            "content": post.content,
            ...
            if len(Reaction.objects.filter(assigned_post=post, assigned_user=user)) > 0
            else -1,
        }
        ...
        for comment in comments:
            info = {
                ...
                "assigned_user_avatar": request.build_absolute_uri(
                    comment.assigned_user.avatar.url
                ),
                "time": comment.time,
            }
    ...
    def create(self, request, *args, **kwargs):
        ...
        post_response = {
            ...
            "assigned_user_avatar": "http://"
            + request.get_host()
            + new_post.assigned_user.avatar.url,
        ...
        new_notification = Notification(
            assigned_post=new_post, assigned_user=request.user, type=0
        )
        ...
        for user in user_list.all():
            ...
            if user.id == request.user.id:
                continue
            new_notification_member = NotificationMember(
                assigned_user=user, assigned_notification=new_notification
            )
        ...
        send_to_onesignal_worker(
            APP_ID,
            relation_device_id_list,
            "Đây là notification từ post {}".format(new_post.id),
        )
